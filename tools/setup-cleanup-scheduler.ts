...
const APPS_SCRIPT_DIR = join(import.meta.dirname ?? '.', '..', 'apps-script', 'cleanup-scheduler');
 ...
 *
 * この関数は tools/setup-cleanup-scheduler.ts から clasp run 経由で実行されます。
 * ファイルの内容は deno task gas:apply-cleanup-scheduler 実行時に自動的に上書きされます。
 *
