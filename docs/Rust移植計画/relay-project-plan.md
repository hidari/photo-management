...
- Free: $0/月、100 DB、5GB ストレージ
- Developer: $4.99/月、DB 数無制限、アクティブ DB 500 まで（超過 $0.20/DB）、9GB ストレージ
- 非アクティブ DB はアクティブ DB としてカウントされないため、実質的にはかなりのユーザー数を捌ける
- 開発・テスト段階は Free プランで十分。本番リリース後は Developer プラン以上が必要

#### セキュリティとアクセス制御

SQLite / libSQL には PostgreSQL の RLS（行レベルセキュリティ）に相当する機能がない。Turso 独自の「Fine-Grained Permissions」はテーブル/操作レベルの制御であり、行レベルではない。

そのため、マルチユーザー環境でのデータ分離は **Database per Tenant パターン**（物理的に DB を分ける）が Turso の公式推奨。共有 DB で `WHERE user_id = ?` するアプローチはアプリ層のバグで他ユーザーのデータが漏れるリスクがあるため採用しない。

#### データベース粒度設計

```
photographer_{id}.db       ... カメラマンごとの配布・受信者・写真メタデータ
relay_session_{session_id}.db  ... 加工リレーセッション（参加者間で共有）
```

カメラマンごとの DB:
- 1カメラマン = 1 DB。配布（Distribution）、受信者（Recipient）、写真メタデータ（Photo）を格納
- カメラマン A のデータがカメラマン B に見えないことが DB 分離で物理的に保証される
- Turso トークンを「そのカメラマンの DB のみ」にスコープ限定して発行する
- Embedded Replica を使う場合、自分の DB だけがデバイスにレプリカされる（他人のデータが物理的に存在しない）

加工リレーセッションの DB:
- 1 セッション = 1 DB。参加者全員がこの DB を共有する
- セッション作成時に DB を生成し、参加者全員にこの DB へのトークンを発行する
- セッション完了後は DB を削除（またはアーカイブ）してアクティブ DB 数を節約

トークン管理:
- JWT ベースのトークン。DB ごとにスコープ限定して発行可能
- `--read-only` フラグで読み取り専用に制限可能
- 有効期限の設定が可能（`7d`, `30d` 等）
- デバイスごとに個別のトークンを発行し、必要に応じて無効化できる

#### スキーママイグレーションの課題

- Turso の Multi-DB Schemas（親 DB でスキーマ定義 → 子 DB に適用）は deprecated 方向
- ATTACH Database（クロス DB クエリ）も deprecated 方向
- スキーママイグレーションを全テナント DB に適用する仕組みを自前で実装する必要がある
  - 方針: マイグレーション管理用のメタ DB を持ち、各テナント DB にマイグレーションを順次適用するスクリプトを作る
  - Phase 2 実装時に詳細設計する
...
- 画像ストレージはカメラマンの Google Drive 上に作成。受信側は Phase 2 でサーバー仲介方式を採用
- 順番制のため、状態管理が重要。加工リレーセッション DB + Embedded Replica で各参加者のデバイスに同期
- 通知が必須（順番が来たことを知らせる必要がある）→ FCM で実装
