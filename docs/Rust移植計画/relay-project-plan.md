...
### ユーザー識別（確定: Googleアカウント連携）

Google Drive の OAuth 認証と統一し、Googleアカウントでユーザーを識別する。

#### 方針

- アプリ利用時は Googleアカウントでのログインが必須
- 共有リンクを受け取るだけ（ブラウザで閲覧）の場合は認証不要
- 機種変更時のデータ移行は「新デバイスで再ログインするだけ」で完了
- 複数デバイスでも同一の Googleアカウントで識別される

#### メリット

- Google Drive の認証と統一されるため、追加の認証フローが不要
- 機種変更時のデータ移行が最もシンプルな UX
- Turso のユーザー識別にも Google ID をそのまま使える
