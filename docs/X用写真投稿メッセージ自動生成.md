...
1. リポジトリの `apps-script/src/message-generator.ts` ファイルを開く
2. 全内容をコピー
3. Apps Script エディタの `コード.gs` にペースト
4. 「保存」アイコンをクリック（Ctrl/Cmd + S）

> **注意**: TypeScriptで記述されていますが、GASエディタはJavaScriptとして実行します。型注釈はコメントとして無視されます。
...
3. **メッセージ生成**: 「📝 メッセージ生成」→「メッセージを生成」を実行
...
A: はい。同じ行に対して再度メッセージ生成を実行すると、MESSAGE列の内容は上書きされます。
...
- 複数テンプレート対応
- 生成ログの記録
- 自動投稿機能（Playwright + Deno）
- 複数SNS対応（LINE, Discord, Blueskyなど）
